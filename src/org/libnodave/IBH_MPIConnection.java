...
//		System.out.println("readIBHPacket  res:" + res);
	...
	*/
	...
	int resp3[] =
		{ 0xff, 0x07, 0x02, 0x01, 0x97, 0x00, 0x00, 0x00, 0x114, 0x100, };
...
//		    System.out.println("trying next ID:\n");
		...
		if ((Nodave.Debug & Nodave.DEBUG_CONNECT) != 0)
			System.out.println(
				"daveInitStepIBH 5:" + a + " connID: " + dst_conn);
		if (a > 3)
			return -5;
		...
		if ((Nodave.Debug & Nodave.DEBUG_CONNECT) != 0)
			System.out.println(
				"_daveInitStepIBH 5a: " + a + " connID: " + dst_conn);
		if (a > 3)
			return -5;
...
//		System.out.println("enter analyze");
		...
		if (msgIn[6] == (byte) 0x82) {
			...
			if (msgIn[8 + 5] == (byte) 0xb0) {
				System.out.println(
					"Ackknowledge for packet number: " + msgIn[3]);
...
//				System.out.println("found MPI ackknowledge");
				if ((Nodave.Debug & Nodave.DEBUG_RAWREAD) != 0) {
					...
					System.out.println(
						"Ackknowledge for packet number: " + msgIn[3 + 2]);
				}
//						return _davePtMPIAck;			
			} else {
				System.out.println(
					"Unsupported MPI function code !!: %d\n"
						+ msgIn[8
						+ 5
						+ 2]);
				sendMPIAck_IBH();
